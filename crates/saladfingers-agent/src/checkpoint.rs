...
//!   contents changed since the last upload. On exec end the watcher does one final
//!   upload (the writer is gone, so the directory is settled) and exits.
//!
...
/// a final upload and exits. `restored` carries what [`restore`] found, so the first
...
async fn watch_loop(
    ...
    loop {
        ...
        if let Some(mtime) = latest_mtime(dir) {
            ...
            // checkpoint unusable, so keep the previous one instead.
            let torn = stopping
